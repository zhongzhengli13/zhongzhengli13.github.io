<div class="timeline">
  <article class="timeline-item">
    <time>2026.09.20（计划）</time>
    <div>
      <h3>数学建模比赛</h3>
      <p>计划参加数学建模比赛，进一步锻炼建模、分析与协作能力。</p>
    </div>
  </article>

  <article class="timeline-item">
    <time>2026.09 - 至今</time>
    <div>
      <h3>梅花桩与全身运控跑酷探索</h3>
      <p>开始尝试梅花桩等复杂地形任务，并探索真正面向人形机器人的全身运动控制与跑酷技能。</p>
    </div>
  </article>

  <article class="timeline-item">
    <time>2026.06.18</time>
    <div>
      <h3>本科毕业</h3>
      <p>
        毕业于内蒙古农业大学计算机科学与技术专业。
        <span class="hover-gallery" data-images='["static/assets/img/graduation_1.jpg","static/assets/img/graduation_2.jpg","static/assets/img/graduation_3.jpg"]'><span class="hover-gallery-trigger">查看毕业照片</span></span>
      </p>
    </div>
  </article>

  <article class="timeline-item">
    <time>2026.04 - 至今</time>
    <div>
      <h3>双足机器人感知与运控融合</h3>
      <p>开始聚焦双足机器人感知与运控融合，致力于实现复杂环境下的鲁棒自适应行走。相关 Demo 已在<a href="#projects">项目</a>中展示。</p>
    </div>
  </article>
</div>
